...
If the post teaches a catalyst playbook rather than a chart trigger, preserve the playbook as its own framework. Pre-earnings posts can define entry timing, conviction thresholds, sizing caps, total basket exposure, scenario exits, and event-day behavior even when they do not specify one mechanical chart pattern.
...
- Preserve catalyst-event playbooks as execution frameworks. If a master defines when pre-earnings positioning is allowed, how much capital can be allocated per name and across the basket, and exactly how different earnings outcomes should be handled, store those as reusable planning and exit rules instead of forcing them into a simple entry pattern.
...
- Preserve explicit intraday cooling-off language such as a `string of red` on the `30-minute` chart as a tactical reset inside strength, not as evidence that the broader bullish setup is broken.
- Preserve lower-timeframe risk definitions when the master gives them. If the entry is the `30 minute pivot`, keep the stop logic tied to the recent `30-minute` low or equivalent intraday invalidation point instead of flattening it into a generic daily stop.
- Preserve numeric `High Tight Flag` requirements exactly when the master states them. A move of `100%` or more in `8 weeks or less` and a correction of less than `25%` should remain explicit gating criteria for the `HTF` family rather than being paraphrased as generic momentum.
